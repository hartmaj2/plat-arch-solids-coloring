...
For many cases, we have multiple equivalence classes for particular values of $n$, where $n$ is the number of independent sets in the partition. In such cases, we will organize the respective partitions in following figures by a so called \textit{sizes vector}.

\begin{defn}[sizes vector]
    Let $G=(V,E)$ be a graph and let $W$ be a partition of $V$ into $n$ independent sets, where $n$ is some natural number. Let $s=(\abs{I_1}, \ldots, \abs{I_n})$ be a vector s.t. $\forall i: I_i \in W$, $\forall i \neq j: I_i \neq I_j$ and $\forall i \leq j : \abs{I_i} \leq \abs{I_j}$. We call this the \emph{sizes vector} of partition $W$.
\end{defn}
...
\begin{figure}[H]
    ...
    \caption{Equivalence classes of $\rightleftharpoons$ of octahedron for $n$ from $3$ up to $6$.}
    \label{fig:octahedron-relaut-classes}
\end{figure}

\subsection{Cube relabeling-automorphism classes}

In the case of cube and $n=3$, we have three non-equivalent partitions s.t. their sizes vectors are pairwise different. See the following figure: 
...
The sizes vector is an ordered vector of sizes of the independent sets in $W$.
