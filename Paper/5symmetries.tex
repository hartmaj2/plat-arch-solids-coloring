...
\begin{thm}[computation of orbital chromatic polynomial]
...
\begin{proof}
    ...
    In order to prove the equation above, it is enough to show that $\forall \pi \in \Aut(G) : P(G/_{\pi},n) = \abs{\Fix(\pi)}$. Formally, we need to find a bijection $f: \Fix(\pi) \rightarrow C'_n$ where $C'_n$ is the set of all colorings of $G/_{\pi}$ using at most $n$ colors. Given a coloring $c \in \Fix(\pi)$, what can we say about $c$?
\end{proof}
