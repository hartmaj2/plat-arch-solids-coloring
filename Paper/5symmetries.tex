...
Notice, that even though we managed to overcome the problem of counting a single partition $W$ of $G$ into independent sets multiple times, because of the ways we can relabel the colorings, the method still suffers from the fact, that rotated or reflected independent sets might be counted as different objects. This happens exactly in the cases, when rotation or reflection of a coloring $c_1$ corresponding to some labeling of independent sets in partition $W$ leads to a coloring $c_2$ s.t. $c_1 \not\leftrightarrow c_2$. In other words, when by rotating or reflecting the colored graph we get a coloring to which we cannot get by relabeling the colors in the original coloring.

Overcoming this problem motivates the following definition:
...
For example consider the graph of cube and its 4-colorings. 
