...
\todo[JF, noinline]{proč demonstrative?}
...
Note, that we can use the same method as in section \ref{sec:num-clrings-exactly-n-clrs}, using the formula \ref{eqn:exactly-n-colors}, also for the orbital chromatic polynomial $OP(G,x)$, to arrive at corresponding values $OP^*(G,n)$. Using the formula\todo[JF, noinline]{kterou?}, we can  simplify table \ref{tab:platonic-polys-evals} to get the following table:
