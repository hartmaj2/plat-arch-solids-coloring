...
\begin{proof}
    ...
    For surjectivity, suppose we have a proper coloring $c' \in C'_n$ of graph $G/_\pi$. Then the preimage of $c'$ is the coloring $c \in C_n$ as follows: $(\forall i \in \{1, \ldots, k\})( \forall v \in O_i) : c(v) = c'(w_i)$. Again by the definition of the vertex set identification if there exist $i \neq j$ and vertices $v_i \in O_i$, $v_j \in O_j$ s.t. $\{v_i,v_j\} \in E$ then also $\{w_i,w_j\} \in E'$ so $c'(w_i) \neq c'(w_j)$. Also note that if for any $O_i \in \Cycles{\pi}$ and non-equal $u,v \in O_i$ we have a loop $\{w_i\} \in E'$ which is contradiction with $c'$ being a proper coloring. So the coloring $c$ is proper and since $\forall O_i \in \Cycles(\pi)$ and any two $u,v \in O_i : c(u) = c(v)$ the coloring $c$ is also fixed by $\pi$ i.e. $c \in \Fix(\pi)$.

    So we have that $\forall \pi \in \Aut(G) : P(G/_{\pi},n) = \abs{\Fix(\pi)}$ which concludes the proof.
\end{proof}
