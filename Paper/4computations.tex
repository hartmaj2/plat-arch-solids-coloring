...
\label{lemma:non-edge_set_size}
    Let $G = K_{k \times (<n)}$ be a complete $k$-partite graph where each independent set has size at most $n$. Then, for any non-edge $\bar{e} \in \bar{E}(G)$ and $o \in \{\oplus,\star\}$, we have that: 
    \[
