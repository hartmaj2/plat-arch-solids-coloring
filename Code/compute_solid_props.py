...
def get_num_vtces(solid_data : dict[list]):
    return len(solid_data[sdp.JSON_VTCES_PROPERTY_KEY_NAME])

def get_num_edges(solid_data : dict[list]):
    return len(solid_data[sdp.JSON_EDGES_PROPERTY_KEY_NAME])
...
def main():
    ...
    printing.print_solid_mult_col_data(plat_data,sdp.PLATONIC_FOLDER_NAME,DATA_HEADERS)

    archimedean = sdp.get_archimedean_solid_dict()
    arch_data = get_v_e_f_d_dict(archimedean)
    printing.print_solid_mult_col_data(arch_data,sdp.ARCHIMEDEAN_FOLDER_NAME,DATA_HEADERS)
